...
| 6 | Real parent delegation | `BLOCKED_BY_DNS_OPERATOR` | Public parent authority is identified as Internet Naming Co./Tucows TRS; all four parent NS agree on SOA. The existing `1.country` child delegation is directly confirmed at all four parents, but it uses two Cloudflare nameservers and is not the required disposable three-server PowerDNS probe. | Provide actual `ns1`/`ns2`/`ns3`, a delegated disposable `.country` probe, authorized parent-change evidence, and a verified delegation bundle. |
